...
public class FileUpAndDown {
	...
	public static void doUpload(byte[] buffer, String targetName) throws Exception {
		//String dirPath = FileUpAndDown.class.getResource("/").getFile().toString() + CommonUtils.getIPConfigBykey("imagePath");
		String dirPath = CommonUtils.getIPConfigBykey("imagePath");
		if (dirPath != null && !dirPath.equals("")) {
			File folder = new File(dirPath); 
			if (!folder.exists()) {
				folder.mkdirs();
			}
			FileOutputStream out = new FileOutputStream(dirPath + targetName);
			out.write(buffer);
			out.close();
		}
