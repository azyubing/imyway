...
<!-- 
	 --><div class="dropdown bottom-btns mr_10" style="margin-left: 10px;">
		  <button class="btn btn-default dropdown-toggle" type="button" id="dropdownMenu3" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
		    <font>包含天数</font>
		    <input type="hidden" id="daycnt"/>
		    <span class="caret"></span>
		  </button>
		  <ul class="dropdown-menu" id="daycntMenu" aria-labelledby="dropdownMenu3">
		  	<li><a href="javascript:void(0)" id="">请选择</a></li>
		    <c:forEach var="item" varStatus="statusIdx"  begin="1" end="${daycnt[0].daycnt }">
		  		<li><a href="javascript:void(0)" id="${item }">${item }天</a></li>
		  	</c:forEach>
		  </ul>
		</div><!-- 
	 --><div class="dropdown bottom-btns mr_10" style="margin-left: 10px;">
		  ...
		  <ul class="dropdown-menu" id="staylvlMenu" aria-labelledby="dropdownMenu4">
		  	<li><a href="javascript:void(0)" id="">请选择</a></li>
	  		...
	  		<li><a href="javascript:void(0)" id="99">无住宿</a></li>
	 ...
	 --><div class="dropdown bottom-btns mr_10">
		  ...
		  <ul class="dropdown-menu" id="propertyMenu" aria-labelledby="dropdownMenu5">
		  	<li><a href="javascript:void(0)" id="">请选择</a></li>
		  	<li><a href="javascript:void(0)" id="1">一地</a></li>
	  		<li><a href="javascript:void(0)" id="2">多地</a></li>
		    <%-- <c:forEach var="item" items="${property }" varStatus="statusIdx" >
		  		<li>
		  			<a href="javascript:void(0)" id="${item.property }">
						<c:if test="${item.property == 1 }">一地</c:if>
						<c:if test="${item.property == 2 }">多地</c:if>
					</a>
		  		</li>
		  	</c:forEach> --%>
		  </ul>
...
jQuery(document).ready(function() {
	...
	//包含天数
	$("#daycntMenu>li>a").click(function(){
	...
	//套餐性质
	$("#propertyMenu>li>a").click(function(){
		var tid = $(this).parent().parent().attr("aria-labelledby");
		var countryId = $(this).attr('id');
		$("#"+tid).find("font").text($(this).text());
		$("#"+tid).find("input").val(countryId);
...
function cleanBtn(){
	...
	$("#daycnt").val('');
	$("#daycnt").prev().text('包含天数');
	
	$("#staylvl").val('');
	$("#staylvl").prev().text('住宿等级');
	
	$("#property").val('');
	$("#property").prev().text('套餐性质 ');
...
function seachPackageList(){
	...
	$(theme).each(function(index){
		...
		$.ajax({
			type: "GET",	
			async: true,
			url: "<%=request.getContextPath()%>/product/package_list_ajax.html?"+new Date(),
			data: {'pname': $("#pname").val(),'country': $("#country").val(),'city': $("#city").val(),
				'tags': $("#tags").val(),'startTime': $("#startTime").val(),'endTime': $("#endTime").val(),
				'daycnt': $("#daycnt").val(),'property': $("#property").val(),'staylvl': $("#staylvl").val(),
			...
			success: function(json){
				...
				$(objectList).each(function(index){
					...
					var toHref = '<%=basePath%>product/package_detail.html?pid='+objectList[index].pid;
					...
					objectLebel.append(
							...
							'			<p style=" overflow: hidden; text-overflow: ellipsis;white-space: nowrap;" title="'+objectList[index].pname+'" >'+objectList[index].pname+'</p>'+
