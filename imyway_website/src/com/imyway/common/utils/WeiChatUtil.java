...
public class WeiChatUtil {
	...
	private String getUserInfo = "https://api.weixin.qq.com/sns/userinfo?access_token={0}&openid={1}";
	/**
	 ...
	 */
	public Map<String, Object> getOauth2Info(String appid, String secret,
			String code) {
		try {
			log.info("getOauth2Info start.{code:" + code + "}");
			String url = MessageFormat.format(this.getOauth2Url, appid, secret,
					code);
	...
	public Map<String, Object> getUserInfo(String accessToken, String openId) {
		try {
			...
			String url = MessageFormat.format(this.getUserInfo, accessToken,
					openId);
			String response = executeHttpGet(url);
			Map<String, Object> map = doJson2Map(response);
	...
	public Map<String, Object> validUserToken(String openid, String token) {
		try {
			log.info("validUserToken start.{openid:" + openid
					+ ",access_token:" + token + "}");
			String url = MessageFormat.format(this.validUserTokenUrl, token,
					openid);
			String response = executeHttpGet(url);
			Map<String, Object> map = doJson2Map(response);
	 ...
	 */
	private Map<String, Object> doJson2Map(String response) throws IOException,
